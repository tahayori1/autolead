...
import { LeadSessionInfo } from '../services/api';
...
interface UserTableProps {
    ...
    leadActivitiesMap?: Record<number, { author: string; authorFullName: string; type: 'journal' | 'call' | 'survey' }>;
    activeSessions?: LeadSessionInfo[];
}

const UserTable: React.FC<UserTableProps> = ({ 
    users, onEdit, onDelete, onViewDetails, onSort, sortConfig, 
    selectedUserIds, onSelectionChange, onSelectAllChange,
    onRegisterOrder, loggedInUser, leadActivitiesMap = {}, activeSessions = []
}) => {
        ...
        <div className="bg-transparent md:bg-white md:dark:bg-slate-800 md:rounded-[24px] md:shadow-sm md:border md:border-slate-200 md:dark:border-slate-700 overflow-hidden">
            ...
            <div className="hidden md:block overflow-x-auto">
                <table className="w-full text-sm text-right text-slate-600 dark:text-slate-300">
                    ...
                    <tbody>
                        {users.map((user) => {
                            const activity = leadActivitiesMap[user.id];
                            const activeViewers = activeSessions.filter(s => s.leadId === user.id && s.username !== loggedInUser?.username);
                                ...
                                `}>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-sky-100 to-blue-100 dark:from-sky-900 dark:to-blue-900 text-sky-700 dark:text-sky-300 flex items-center justify-center text-xs font-bold shadow-sm flex-shrink-0">
                                            ...
                                            <div className="flex flex-col gap-1">
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    ...
                                                    {activity && (
                                                        <span 
                                                            className="inline-flex items-center gap-0.5 text-rose-600 dark:text-rose-400 text-[10px] bg-rose-50 dark:bg-rose-950/40 px-1.5 py-0.5 rounded-md border border-rose-100 dark:border-rose-900/40 font-black"
                                                            title={`این سرنخ قفل شده است زیرا ${activity.authorFullName} قبلاً فعالیتی روی آن ثبت کرده است.`}
                                                        >
                                                            🔒 {activity.authorFullName}
                                                        </span>
                                                    )}
                                                    {activeViewers.length > 0 && (
                                                        <span 
                                                            className="inline-flex items-center gap-0.5 text-amber-600 dark:text-amber-400 text-[10px] bg-amber-50 dark:bg-amber-950/40 px-1.5 py-0.5 rounded-md border border-amber-100 dark:border-amber-900/40 font-black animate-pulse"
                                                            title={`هم‌اکنون توسط ${activeViewers.map(v => v.fullName).join(' و ')} در حال مشاهده است.`}
                                                        >
                                                            👀 {activeViewers[0].fullName}
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
                                    ...
                                    <div className="flex justify-between items-start gap-1">
                                        <div className="flex flex-col gap-1 min-w-0">
                                            <div className="flex items-center gap-1.5 flex-wrap">
                                                ...
                                                {activity && (
                                                    <span 
                                                        className="inline-flex items-center gap-0.5 text-rose-600 dark:text-rose-400 text-[10px] bg-rose-50 dark:bg-rose-950/40 px-1 py-0.5 rounded border border-rose-100 dark:border-rose-900/40 font-bold"
                                                    >
                                                        🔒 {activity.authorFullName}
                                                    </span>
                                                )}
                                                {activeViewers.length > 0 && (
                                                    <span 
                                                        className="inline-flex items-center gap-0.5 text-amber-600 dark:text-amber-400 text-[10px] bg-amber-50 dark:bg-amber-950/40 px-1 py-0.5 rounded border border-amber-100 dark:border-amber-900/40 font-bold animate-pulse"
                                                    >
                                                        👀 {activeViewers[0].fullName}
                                                    </span>
                                                )}
